...
def resnet34_setter_iterator(freeze=False):
    ...
    def setter05(model, layer):
        model.layer1[2].conv1 = layer

    def setter06(model, layer):
        model.layer1[2].conv2 = layer
    ...
    def setter12(model, layer):
        model.layer2[2].conv1 = layer

    def setter13(model, layer):
        model.layer2[2].conv2 = layer

    def setter14(model, layer):
        model.layer2[3].conv1 = layer

    def setter15(model, layer):
        model.layer2[3].conv2 = layer
    ...
    def setter20(model, layer):
        model.layer3[1].conv2 = layer

    def setter21(model, layer):
        model.layer3[2].conv1 = layer

    def setter22(model, layer):
        model.layer3[2].conv2 = layer

    def setter23(model, layer):
        model.layer3[3].conv1 = layer

    def setter24(model, layer):
        model.layer3[3].conv2 = layer

    def setter25(model, layer):
        model.layer3[4].conv1 = layer

    def setter26(model, layer):
        model.layer3[4].conv2 = layer

    def setter27(model, layer):
        model.layer3[5].conv1 = layer

    def setter28(model, layer):
        model.layer3[5].conv2 = layer

    def setter29(model, layer):
        model.layer4[0].conv1 = layer

    def setter30(model, layer):
        model.layer4[0].conv2 = layer

    def setter31(model, layer):
        model.layer4[0].downsample = layer

    def setter32(model, layer):
        model.layer4[1].conv1 = layer

    def setter33(model, layer):
        model.layer4[1].conv2 = layer

    def setter34(model, layer):
        model.layer4[2].conv1 = layer

    def setter35(model, layer):
        model.layer4[2].conv2 = layer
...
def resnet34_getter_iterator(freeze=False):
    if freeze:
    ...
    yield attrgetter('layer1.2.conv1')
    yield attrgetter('layer1.2.conv2')
    ...
    yield attrgetter('layer2.2.conv1')
    yield attrgetter('layer2.2.conv2')
    yield attrgetter('layer2.3.conv1')
    yield attrgetter('layer2.3.conv2')
    ...
    yield attrgetter('layer3.2.conv1')
    yield attrgetter('layer3.2.conv2')
    yield attrgetter('layer3.3.conv1')
    yield attrgetter('layer3.3.conv2')
    yield attrgetter('layer3.4.conv1')
    yield attrgetter('layer3.4.conv2')
    yield attrgetter('layer3.5.conv1')
    yield attrgetter('layer3.5.conv2')
    ...
    yield attrgetter('layer4.2.conv1')
    yield attrgetter('layer4.2.conv2')


def ptq_resnet34(fresnet, in_bins: int, w_bins: int, symmetric_mode: bool, train_loader, device='cpu', mode='ada'):
    fresnet = fresnet.to(device)
    guide = MyQResNet([3, 4, 6, 3]).to(device)
    ...
    in_step = -1
    qresnet = MyQResNet([3, 4, 6, 3]).to(device)
    qresnet.load_state_dict(fresnet.state_dict())
    qresnet = qresnet.to(device)
    set_itter = resnet34_setter_iterator()
    for i, layer_getter in enumerate(resnet34_getter_iterator()):
...
def get_qresnet_34(original_model, pretrained=True, device='cpu'):
    model = MyQResNet([3, 4, 6, 3]).to(device)
    if pretrained:
        paired_layers = [
            ...
            (model.layer1[2].conv1.conv, original_model.layer1[2].conv1),
            (model.layer1[2].conv1.bn,   original_model.layer1[2].bn1),
            (model.layer1[2].conv2.conv, original_model.layer1[2].conv2),
            (model.layer1[2].conv2.bn,   original_model.layer1[2].bn2),
            ...
            (model.layer2[2].conv1.conv, original_model.layer2[2].conv1),
            (model.layer2[2].conv1.bn,   original_model.layer2[2].bn1),
            (model.layer2[2].conv2.conv, original_model.layer2[2].conv2),
            (model.layer2[2].conv2.bn,   original_model.layer2[2].bn2),
            (model.layer2[3].conv1.conv, original_model.layer2[3].conv1),
            (model.layer2[3].conv1.bn,   original_model.layer2[3].bn1),
            (model.layer2[3].conv2.conv, original_model.layer2[3].conv2),
            (model.layer2[3].conv2.bn,   original_model.layer2[3].bn2),
            ...
            (model.layer3[2].conv1.conv, original_model.layer3[2].conv1),
            (model.layer3[2].conv1.bn,   original_model.layer3[2].bn1),
            (model.layer3[2].conv2.conv, original_model.layer3[2].conv2),
            (model.layer3[2].conv2.bn,   original_model.layer3[2].bn2),
            (model.layer3[3].conv1.conv, original_model.layer3[3].conv1),
            (model.layer3[3].conv1.bn,   original_model.layer3[3].bn1),
            (model.layer3[3].conv2.conv, original_model.layer3[3].conv2),
            (model.layer3[3].conv2.bn,   original_model.layer3[3].bn2),
            (model.layer3[4].conv1.conv, original_model.layer3[4].conv1),
            (model.layer3[4].conv1.bn,   original_model.layer3[4].bn1),
            (model.layer3[4].conv2.conv, original_model.layer3[4].conv2),
            (model.layer3[4].conv2.bn,   original_model.layer3[4].bn2),
            (model.layer3[5].conv1.conv, original_model.layer3[5].conv1),
            (model.layer3[5].conv1.bn,   original_model.layer3[5].bn1),
            (model.layer3[5].conv2.conv, original_model.layer3[5].conv2),
            (model.layer3[5].conv2.bn,   original_model.layer3[5].bn2),
            ...
            (model.layer4[2].conv1.conv, original_model.layer4[2].conv1),
            (model.layer4[2].conv1.bn,   original_model.layer4[2].bn1),
            (model.layer4[2].conv2.conv, original_model.layer4[2].conv2),
            (model.layer4[2].conv2.bn,   original_model.layer4[2].bn2),
...
def gradual_freeze_resnet34(qresnet, device):
    # FREEZE_ITERS = 50
    # TEST_ITERS = 100
    frozen_resnet = MyQResNet([3, 4, 6, 3], quantized=True).to(device)
    frozen_resnet.load_state_dict(qresnet.state_dict())
    frozen_resnet = frozen_resnet.to(device)

    train_stat, test_stat = None, None
    set_itter = resnet34_setter_iterator(freeze=True)
    for layer_getter in resnet34_getter_iterator(freeze=True):
        layer_setter = next(set_itter)
        qlayer = layer_getter(qresnet)
...
def main(prefix, imagenet_train_path, imagenet_val_path, pretrained_model, finetune, device, n_exp_rep):
    BATCH_SIZE = 64
    ...
    if pretrained_model is not None:
        torch_resnet = resnet34()
        print(f'Loading {pretrained_model}')
        torch_resnet.load_state_dict(torch.load(pretrained_model))
    else:
        torch_resnet = resnet34(weights=ResNet34_Weights.IMAGENET1K_V1).to(device)
    torch_resnet = torch_resnet.to(device)
    fqresnet = get_qresnet_34(torch_resnet, device=device)
    print('Loaded resnet')
    ...
    for j in range(n_exp_rep):
        def quantization(in_bins, weight_bins, symmetric=True):
            ...
            qqresnet = ptq_resnet34(fqresnet, in_bins, weight_bins, symmetric, train_loader, device=device)
            save_model(qqresnet, f'{name}_{in_bins}_{weight_bins}_ptq__{j + 1}', test_loader, device=device)
            frozen_resnet, train_stat, test_stat = gradual_freeze_resnet34(qqresnet, device=device)
            quant_end = time.time()
